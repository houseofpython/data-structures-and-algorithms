...
const containsW = (str) => {
  // Solution code here...
  let regex = /w/g;
  let doesItHaveW = regex.test(str);
  return doesItHaveW;
};
...
const containsWorld = (input) => {
  // Solution code here...
  const regex = /world/g;
  const doesItHaveWorld = regex.test(input);
  return doesItHaveWorld
};
...
const isCapitalized = (str) => {
  // Solution code here...
  const regex = str.match(/[A-Z][a-zA-Z]*/gm) || [];
  return regex;
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  const namesAthruJ = /^[A-J]/;
  let newArray = [];
  arr.forEach(element => {
    if (namesAthruJ.test(element)) newArray.push(element);
  });
  return newArray;
};
